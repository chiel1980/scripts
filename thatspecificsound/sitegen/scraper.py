...
# Naive but effective sentence boundary: punctuation followed by whitespace
# and a capital letter. Good enough here since we only need to find *where*
# one sentence ends and the next begins, not a linguistically perfect split.
_SENTENCE_BOUNDARY_RE = re.compile(r'(?<=[.?!])\s+(?=[A-Z\u201c"])')


def split_qa_segments(text: str) -> list[dict]:
    """Split `text` into a list of {"kind": "question"|"answer", "text": ...}
    segments, sentence by sentence, merging consecutive sentences of the
    same kind into one segment.

    This exists because the source site doesn't always put a paragraph
    break between the end of one answer and the start of the next
    question -- both can arrive as a single element's worth of text. A
    sentence is treated as a question if it ends in "?", same rule as
    before, just applied per-sentence instead of to the whole element so a
    trailing answer sentence or a leading question sentence glued onto a
    neighbor doesn't drag the entire block into the wrong classification.
    """
    ...
    segments: list[dict] = []
    current_kind = "question" if sentences[0].endswith("?") else "answer"
    current_parts = [sentences[0]]
    for sentence in sentences[1:]:
        kind = "question" if sentence.endswith("?") else "answer"
        if kind != current_kind:
            segments.append({"kind": current_kind, "text": " ".join(current_parts)})
            current_parts = []
            current_kind = kind
...
def parse_interview(html: str, url: str) -> dict:
    ...
    if content:
        ...
        #
        # "figcaption" is also included: WordPress's block-editor image
        # blocks put the caption -- which is exactly where a "Photo
        # credits: ..." line usually lives -- in a <figcaption> sibling
        # of the <img>, not in a <p>. Older/"classic editor" posts instead
        # use <p class="wp-caption-text">, which was already covered since
        # it's a plain <p>; figcaption was the one real gap.
        for el in content.find_all(["p","h2","h3","li","blockquote","img","figcaption"], recursive=True):
            if el.name == "img":
                src = el.get("src")
            ...
            if is_question_whole:
                blocks.append({"kind":"question","text":text})
                continue

            # Several source pages (e.g. the Scott Mellinger interview) have
            # no paragraph break between the end of one answer and the start
            # of the next question -- WordPress served both as a single <p>.
            # Classifying by "does the *whole* element end in '?'" then
            # swallows the tail end of the previous answer into the next
            # question block (and colors it yellow along with it), or the
            # reverse: a question glued onto the front of an answer gets
            # rendered white. Split the element into sentences and classify
            # each one individually, merging consecutive same-kind sentences
            # back together, so a glued-together "...upgrade. Can you tell
            # me...?" still comes out as a separate answer sentence followed
            # by a separate question rather than one giant misclassified
            # block. Elements that are already a single clean sentence just
            # produce a single segment, so normal (already-separated) pages
            # are unaffected.
            for seg in split_qa_segments(linked_text):
                if seg["kind"] == "question":
                    # Question blocks are rendered from plain text elsewhere
                    # (see the is_question_whole branch above), so strip any
                    # [label](url) markdown-link markers here too for
                    # consistency -- a question is never expected to carry a
                    # clickable link.
                    plain = re.sub(r"\[([^\]]+)\]\([^)]+\)", r"\1", seg["text"]).strip()
                    blocks.append({"kind":"question","text":plain})
                else:
                    blocks.append({"kind":"answer","text":seg["text"]})
